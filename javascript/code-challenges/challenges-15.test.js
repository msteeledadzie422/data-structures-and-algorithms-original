...
const screenForNames = (arr) => {
  return arr.filter(e => /^(Mr|Mrs|Ms|Dr)\.\ [A-Za-z0-9]+.+/.test(e));
};
...
let biggerThanLuke = (arr) => {
  return arr.filter(e => parseInt(e.mass) > 77).map(r => r.name).join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  ...
  let posSol = [row0, row1, row2, col0, col1, col2, dia1, dia2];
  return posSol.filter(e => e.join('') === 'XXX' || e.join('') === 'OOO').length > 0;
};
